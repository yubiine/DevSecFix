...
function ScanningPage() {
  ...
  const progress = useMemo(() => {
    ...
    return Math.min(96, 16 + stageIndex * 14 + elapsed * 2);
  }, [elapsed, stageIndex, status]);
        ...
        <section className="workflow-card scan-card">
          ...
          <div className="stage-grid">
            {stages.map((stage, index) => (
              <article className={index <= stageIndex ? 'active' : ''} key={stage}>
          ...
          {status === 'failed' ? (
            <div className="inline-alert">
              대상 URL이나 백엔드 로그를 확인한 뒤 다시 시도해 주세요.
            </div>
          ) : (
            <p className="scan-note">
              분석이 끝나면 리포트 화면으로 자동 이동합니다.
            </p>
          )}
        </section>
      </main>
