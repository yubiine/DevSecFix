...
function LoginPage() {
  ...
  return (
    <div className="product-shell login-page">
      <header className="app-topbar"><button className="brand" type="button" onClick={() => navigate('/')}><span>DevSecFix</span></button><button className="back-home" type="button" onClick={() => navigate('/')}>서비스 소개로 돌아가기</button></header>
      <main className="login-layout">
        ...
        <section className="login-card">
          ...
          <label><span>이메일</span><input value={email} onChange={(e) => setEmail(e.target.value)} type="email" /></label>
          <label><span>비밀번호</span><input value={password} onChange={(e) => setPassword(e.target.value)} type="password" /></label>
          {mode === 'signup' && <label><span>비밀번호 확인</span><input defaultValue={password} type="password" /></label>}
          {mode === 'login' && <div className="login-options"><label><input type="checkbox" defaultChecked /> 로그인 유지</label><button type="button">비밀번호 찾기</button></div>}
          <button className="login-submit" type="button" onClick={() => navigate('/dashboard')}>{mode === 'login' ? '로그인' : '무료 계정 만들기'}</button>
          <div className="login-divider"><span>또는</span></div>
          <button className="signup-button" type="button" onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}>{mode === 'login' ? '처음이라면 무료로 시작하기' : '이미 계정이 있다면 로그인'}</button>
